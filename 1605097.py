...
def preprocess_data():
    df =  pd.read_csv('data.csv', names = ['col_1','col_2','col_3','col_4','col_5','col_6',
    'col_7','col_8','label'])
    ...
    scaler = StandardScaler()
    df=scaler.fit_transform(df) 
    train_x,test_x,train_y,test_y = train_test_split(df,y,test_size=.2,random_state = 20)
    model = LogitRegression( learning_rate = 0.2, iterations = 1000 )
    losses=model.fit( train_x, train_y,0,0 )
    # plt.plot(losses)
    # plt.show()	  
    ...
    for i in range(1,5):
        k = i*5
        h, z = adaboost(lr,df,k,'label')
        print ('Round',k)
        print ('Training Dataset')
        adaboost_predict(lr,h,z,'label')
